...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const formInitialTop = form.getBoundingClientRect().top;
...
function onEmailInput(e) {
    ...
    if (curEmailIndex > 0) {
        if (mouthStatus == "small") {
            mouthStatus = "medium";
            TweenMax.to([mouthBG, mouthOutline, tooth], 1, {
...
function getCoord(el) {
    ...
    return {
        x: elCoords.left + elCoords.width / 2,
        y: elCoords.top + elCoords.height / 2
    };
}
...
TweenMax.set([eyeL, eyeR], { transformOrigin: "center center" });

emailCoords = getCoord(email);
eyeLCoords = getCoord(eyeL);
eyeRCoords = getCoord(eyeR);
noseCoords = getCoord(nose);
mouthCoords = getCoord(mouth);
screenCenter = emailCoords.x;
emailScrollMax = email.scrollWidth - email.clientWidth;
